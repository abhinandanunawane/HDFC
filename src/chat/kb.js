...
export function answerGoldLoanFaq(q, ctx) {
  ...
  const common = `\n\nIf you want, I can help you generate a Gold Loan Estimate now.`;

  if (!query) {
    return "Ask me anything about HDBFS Gold Loan: gold rates, interest rates, eligibility, charges, timelines, repayment options, documents, or EMI planning.";
  }

  if (/(interest|rate|roi|apr)/.test(query)) {
    return (
      `Gold Loan interest rates depend on multiple factors like loan amount, tenure, scheme and customer profile.\n\n` +
  ...
  if (/(process|apply|application|steps|timeline|time|how long)/.test(query)) {
    return (
      ...
      bullets([
        ...
        "Visit the branch with your gold for final verification and disbursal",
      ]) +
  ...
  if (/(charges|fee|fees|processing|penal|foreclosure|prepay|late)/.test(query)) {
    return (
      `Charges vary by scheme and branch. Typical charge types include:\n` +
      bullets([
  ...
  if (/(eligib|eligible|who can|requirements|age|documents)/.test(query)) {
    return (
      `High‑level gold loan eligibility:\n` +
      bullets([
  ...
  if (/(ltv|loan to value|75%|how much loan|amount)/.test(query)) {
    return (
      `In this demo, your indicative gold loan eligibility is calculated as:\n` +
  ...
  return (
    `I can help with gold loan FAQs like gold rates, interest rates, process, charges, documents, valuation, and EMI planning.\n\n` +
    `Try asking:\n` +
    bullets([
